...
from bearish.models.assets.base import ComponentDescription  # type: ignore
...
from bearish.utils.utils import to_dataframe  # type: ignore
...
class View(ComponentDescription):  # type: ignore
...
class EuropeanLargeCapLoser(BaseViews):
    query: str = """SELECT symbol, name, source ,
    last_month_max_growth,
    last_year_max_growth FROM analysis
    WHERE positive_free_cash_flow=1
    AND positive_net_income=1
    AND positive_operating_income=1
    AND quarterly_positive_free_cash_flow=1
